...
- Replace `PROTO_NO_UNIQUE_ADDRESS` with a bare `[[__no_unique_address__]]` and drop the MSVC branch. **There is no `_GLIBCXX_NO_UNIQUE_ADDRESS` macro** -- an earlier revision of this file claimed there was. libstdc++ spells the attribute directly, using the reserved form; see `bits/hashtable_policy.h` and `bits/allocated_ptr.h`.
- The attribute is not cosmetic, and there is now a measurement rather than an argument for that: MSVC, whose `[[msvc::no_unique_address]]` does not eliminate the empty deleter, measures `sizeof(hazard_pointer_obj_base)` at **40 against GCC's and clang's 32**. That 8 bytes is exactly the reservation P2530R3 sec. 1.5 asks for, so on a toolchain where the attribute does not apply, the reserved space is paid for twice.
